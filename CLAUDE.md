...
backup_config.json     # Backup password, FTP/SFTP config, scheduled jobs + run history (gitignored)
```
...
`batch_size`: number of per-package report files per zip email (1–100, default 20). For ADOMs with >20 packages, multiple emails are sent: email 1 has the full summary table + Part 1 zip; subsequent emails contain `[Part N of M]` subject and the next zip.

**`bulk_hygiene_adom(adom, checks, include_unused_objects, max_workers=4)`** in `app/routes/hygiene_routes.py` — session-free entry point for the scheduler. Uses `ThreadPoolExecutor(max_workers=4)`. Scope member data from each package object is used to determine the device name for file naming.
...
- Two hooks in `.githooks/` run automatically: `pre-commit` blocks commits containing sensitive strings, and `post-commit` updates the graphify graph. One-time setup per clone: `git config core.hooksPath .githooks`.
