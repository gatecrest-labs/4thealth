        ...
        with make_client() as client:
            ...
            for adom in adom_names:
                try:
                    ...
                    logger.warning("versions_cache: get_devices(%s) failed: %s", adom, exc)

        elapsed = round(_time.monotonic() - t0, 1)
        logger.info("versions_cache: done in %ss — %d devices across %d ADOMs",
...
def refresh_now(app):
    ...
    t = threading.Thread(target=_run_job, args=[app], name="versions_cache_refresh", daemon=True)
    t.start()
...
def init_scheduler(app):
    ...
    t = threading.Thread(target=_run_job, args=[app], name="versions_cache_startup", daemon=True)
    t.start()
