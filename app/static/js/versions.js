...
function exportFGData(format) {
  ...
  if (format === 'csv') {
    ...
    const lines = rows.map(d => {
      const sub = ((d.subscriptions || {})[fgSelSub] || {});
      return [d.name, subLabel, sub.status || 'unknown', sub.expires || '', d.firmware || '', d.adom]
    ...
    const out = rows.map(d => {
      const sub = ((d.subscriptions || {})[fgSelSub] || {});
      return { name: d.name, subscription: subLabel, status: sub.status || 'unknown', expires: sub.expires || null, firmware: d.firmware, adom: d.adom };
    });
    ...
    const tableRows = rows.map(d => {
      ...
      return `<tr><td>${escHtml(d.name)}</td><td>${escHtml(sLbl)}</td><td>${escHtml(sub.expires || '—')}</td><td>${escHtml(d.firmware || 'n/a')}</td><td>${escHtml(d.adom || '—')}</td></tr>`;
    }).join('');
    const win = window.open('', '_blank');
