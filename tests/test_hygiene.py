...
import pytest
from app.hygiene import check_security_profile_gap, find_unused_objects, check_redundant_rules
...
def _p(pid, name, src, dst, svc, action=1, status="enable"):
    return {
        "policyid": pid, "name": name, "action": action, "status": status,
...
def test_redundant_policy_block_skipped():
    ...
    a = _p(1, "rule-a", ["SrcA"], ["DstA"], ["HTTP"])
    b = {**_p(2, "rule-b", ["SrcA"], ["DstA"], ["HTTP"]), "_policy_block": "ThreatBlock"}
    findings = check_redundant_rules([a, b])
    assert findings == []
