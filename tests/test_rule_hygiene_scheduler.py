...
def _pkg_result(name="MyPkg", device="fw-01", n_findings=1, error=None):
    return {
        ...
        "findings": [
            {"policy_id": "1", "policy_name": "allow-all", "seq": 1,
             "check": "unnamed", "detail": "No name"}
        ] * n_findings,
        "unused_objects": None,
        "policy_count": 10,
        "error": error,
...
def test_valid_all_days(tmp_path, monkeypatch):
    monkeypatch.setattr(rhs, "_JOBS_PATH", tmp_path / "jobs.json")
    job = rhs.create_job(_job_data(days_of_week=["SUN","MON","TUE","WED","THU","FRI","SAT"]))
...
def test_execute_single_batch(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("app.smtp_client.load_smtp_config", lambda: {"run_history_days": 30})

    job = rhs.create_job(_job_data(batch_size=20))
    rhs._execute_job(job["id"])

    assert len(sent) == 1
    to, subject, body_html, attachments = sent[0]
...
def test_scope_member_filename_multi():
    result = {
        "package": "MyPkg", "package_name": "MyPkg",
        "device": None, "scope_members": ["fw-01", "fw-02"],
        "findings": [], "unused_objects": None, "policy_count": 5, "error": None,
    }
    filename, _ = rhs._build_attachment_rh(result, "html", "2026-09-01T03:00:00Z")
...
def test_run_history_pruning(tmp_path, monkeypatch):
    ...
    old_run = {
        ...
        "status": "ok", "packages_total": 1, "packages_reviewed": 1,
        "total_findings": 0, "emails_sent": 1, "errors": [],
    }
    rhs._append_run(job["id"], old_run)
    rhs._execute_job(job["id"])

    updated = next(j for j in rhs.get_all_jobs() if j["id"] == job["id"])
    cutoff = (datetime.datetime.utcnow() - datetime.timedelta(days=31)).isoformat()
    assert all(r["ran_at"] > cutoff for r in updated["runs"])


def test_error_in_one_package(tmp_path, monkeypatch):
    monkeypatch.setattr(rhs, "_JOBS_PATH", tmp_path / "jobs.json")
    monkeypatch.setattr(rhs, "_bulk_hygiene_adom",
                        lambda *a, **kw: _make_results(3, error_idx=1))
    monkeypatch.setattr(rhs, "_send_email", lambda *a: None)
    monkeypatch.setattr("app.smtp_client.load_smtp_config", lambda: {"run_history_days": 30})
